...
import stratum_pb2 as protob
import protobuf_mapping

import logger
log = logger.get_logger('protocol-binary')
...
def obj2protob(method, data):
    res = protobuf_mapping.get_class_by_json(method)()
...
def protob2obj(msg):
    method = protobuf_mapping.get_json_by_class(msg)
    res = ''
    return method, res

class ProtocolBinary(IntNStringReceiver):
    structFormat = "!I"
    ...
    def writeRequest(self, payload, is_notification=False):
    ...
    def writeJsonRequest(self, method, params, is_notification=False):
        payload = obj2protob(method, params)
        return self.writeRequest(payload)
...
class ClientProtocolBinary(ProtocolBinary):
    def connectionMade(self):
        ProtocolBinary.connectionMade(self)
